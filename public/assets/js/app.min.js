...
if (fadeArray_items.length) {
	const fadeArray = gsap__WEBPACK_IMPORTED_MODULE_0__["default"].utils.toArray(".fade_anim");

	fadeArray.forEach((item) => {
		// Set default values and read attributes
		const fade_direction = item.getAttribute("data-fade-from") || "bottom";
		const duration_value =
			parseFloat(item.getAttribute("data-duration")) || 0.75;
		const delay_value = parseFloat(item.getAttribute("data-delay")) || 0.15;
		const fade_offset = 40;
		const ease_value = "power2.out";
		...
		// Add ScrollTrigger if necessary
		...
		// Apply animation
		gsap__WEBPACK_IMPORTED_MODULE_0__["default"].from(item, animation_settings);
	});
}
...
function revealAnim(selector) {
	document.querySelectorAll(selector).forEach((item) => {
		const direction = item.getAttribute("data-reveal-from") || "left";
		const delay = parseFloat(item.getAttribute("data-delay")) || 0;
		const duration = parseFloat(item.getAttribute("data-duration")) || 1.2;

		// Set will-change for better performance
		item.style.willChange = "opacity, transform, clip-path";

		// Define initial clip-path values based on direction
		const clipPaths = {
			top: "inset(100% 0% 0% 0%)",
		...
		gsap__WEBPACK_IMPORTED_MODULE_0__["default"].fromTo(
			...
			{
				opacity: 0,
				clipPath: clipPaths[direction] || clipPaths.left, // Default to left if invalid
			},
			{
				opacity: 1,
				...
				onComplete: () => (item.style.willChange = "auto"), // Reset will-change
			},
		);
	});
}
...
function splitTextAnimation(selector) {
	document.querySelectorAll(selector).forEach((item) => {
		const delay = parseFloat(item.getAttribute("data-delay")) || 0;
		const duration = parseFloat(item.getAttribute("data-duration")) || 0.6;

		// Set will-change for better performance
		item.style.willChange = "transform, opacity";

		// Split text into words and letters, wrapping in spans
		const words = item.innerText
			.split(" ")
			.map((word) => {
				return `<span style="white-space: nowrap; display: inline-block; overflow: hidden;">${[
					...word,
				]
					.map(
						(letter) =>
							`<span style="display: inline-block; opacity: 0; transform: translateY(80px) rotateX(120deg) scale(0.8); filter: blur(5px);">${letter}</span>`,
					)
					.join("")}</span>`;
			})
			.join(" ");

		// Update innerHTML in one operation
		item.innerHTML = words;

		const letters = item.querySelectorAll("span span"); // Selects all inner letter spans

		// Defer animation to next frame for better performance
		requestAnimationFrame(() => {
			gsap__WEBPACK_IMPORTED_MODULE_0__["default"].to(letters, {
				opacity: 1,
				y: 0,
				rotationX: 0,
				scale: 1,
				...
				duration: duration,
				delay: delay,
				ease: "power4.out", // Smoother, more dramatic easing
				stagger: {
					each: 0.035 + Math.random() * 0.02, // Slight randomization for a natural feel
					from: "start",
				},
				scrollTrigger: {
					trigger: item,
				...
				onComplete: () => (item.style.willChange = "auto"), // Reset for performance
			});
		});
	});
}
...
function scaleTextReveal(selector) {
	document.querySelectorAll(selector).forEach((item) => {
		const delay = parseFloat(item.getAttribute("data-delay")) || 0;
		const duration = parseFloat(item.getAttribute("data-duration")) || 1.5;

		// Set will-change for better performance
		item.style.willChange = "transform, opacity";

		// Split text into words and wrap letters in span elements
		const words = item.innerText
			.split(" ")
			.map((word) => {
				return `<span style="white-space: nowrap; display: inline-block; overflow: hidden;">${[
					...
					.map(
						(letter) =>
							`<span style="display: inline-block; opacity: 0; transform: translateY(50px) scale(0.8) rotate(-5deg); filter: blur(4px);">${letter}</span>`,
					)
					.join("")}</span>`;
			})
			.join(" ");

		// Update innerHTML in one operation to minimize reflows
		item.innerHTML = words;

		const letters = item.querySelectorAll("span span"); // Select all inner letter spans
		...
		requestAnimationFrame(() => {
			gsap__WEBPACK_IMPORTED_MODULE_0__["default"].to(letters, {
				...
				scale: 1,
				y: 0,
				rotate: 0, // Remove initial tilt smoothly
				filter: "blur(0px)", // Remove blur effect gradually
				duration: duration,
				delay: delay,
				ease: "elastic.out(1, 0.6)", // Smooth but controlled bounce
				stagger: {
					each: 0.035 + Math.random() * 0.02, // Natural variation in timing
					from: "start",
				},
				scrollTrigger: {
					trigger: item,
					start: "top 90%",
					toggleActions: "play none none none",
					once: true, // Only play once on scroll
				},
				onComplete: () => {
					item.style.willChange = "auto"; // Reset for performance
				},
			});
		});
	});
...
// blur text animation
function blurTextReveal(selector) {
	document.querySelectorAll(selector).forEach((item) => {
		...
		const words = item.innerText
			...
			.map((word) => {
				return `<span style="white-space: nowrap; display: inline-block;">${[
					...
					.map(
						(letter) =>
							`<span style="display: inline-block; opacity: 0; filter: blur(10px); transform: translateY(5px);">${letter}</span>`,
					)
					.join("")}</span>`;
			})
			.join(" ");
		...
		// Defer animation to the next frame for smoother transitions
		requestAnimationFrame(() => {
			gsap__WEBPACK_IMPORTED_MODULE_0__["default"].to(letters, {
				opacity: 1,
				y: 0,
				filter: "blur(0px)", // Removes blur effect on letters
				duration: duration,
				delay: delay,
				ease: "power3.out", // Smooth easing for the animation
				stagger: 0.03, // Continuous stagger across all letters for a flowing effect
				scrollTrigger: {
					trigger: item,
					start: "top 85%",
					toggleActions: "play none none none", // Trigger actions for scroll-based animation
					once: true, // Ensures the animation only triggers once
				},
				onComplete: () => {
					item.style.willChange = "auto"; // Reset will-change after animation completes
				},
			});
		});
	});
...
function explodeTextReveal(selector) {
	document.querySelectorAll(selector).forEach((item) => {
		const delay = parseFloat(item.getAttribute("data-delay")) || 0;
		const duration = parseFloat(item.getAttribute("data-duration")) || 0.5;
		...
		textContent.split(" ").forEach((word) => {
			...
			[...word].forEach((letter) => {
				...
				letterSpan.style.willChange = "transform, opacity"; // Performance boost
				wordSpan.appendChild(letterSpan);
			});

			fragment.appendChild(wordSpan);
			fragment.appendChild(document.createTextNode(" ")); // Preserve spaces
		});

		// Replace inner content in one step
		item.innerHTML = "";
		item.appendChild(fragment);

		const letters = item.querySelectorAll("span span");

		// Set initial styles using GSAP to avoid forced reflows during animation
		gsap__WEBPACK_IMPORTED_MODULE_0__["default"].set(letters, {
			opacity: 0,
			transform: "translateY(20px) scale(0.8) rotate(10deg)",
		});

		// Defer animation to the next frame using requestAnimationFrame to prevent forced reflows
		requestAnimationFrame(() => {
			gsap__WEBPACK_IMPORTED_MODULE_0__["default"].to(letters, {
				scale: 1,
				opacity: 1,
				...
				scrollTrigger: {
					...
					start: "top 85%",
					toggleActions: "play none none none",
					once: true, // Prevents multiple reflows
				},
			});
		});
	});
...
function slidingBlurCardReveal(selector) {
	document.querySelectorAll(selector).forEach((item) => {
		...
		// Apply will-change for performance optimization
		item.style.willChange = "transform, opacity, filter";

		// Set initial hidden state
		gsap__WEBPACK_IMPORTED_MODULE_0__["default"].set(item, { opacity: 0, filter: "blur(10px)", y: 50 });

		// Animate the card with smooth transition on scroll
		gsap__WEBPACK_IMPORTED_MODULE_0__["default"].to(item, {
			...
			scrollTrigger: {
				trigger: item,
				start: "top 90%", // Start animation slightly earlier for smoother effect
				end: "bottom 50%", // Keep animation active while in view
				toggleActions: "play none none none",
				once: true, // Ensures the animation triggers only once
				onEnter: () => (item.style.willChange = "auto"), // Reset after animation completes
			},
		});
	});
}

// Initialize the animation on elements
slidingBlurCardReveal(".card_anim");
blurTextReveal(".blur_anim");
scaleTextReveal(".scale_anim");
...
fish.forEach(fishElement => {
	gsap__WEBPACK_IMPORTED_MODULE_0__["default"].from(fishElement, {
		x: fishElement.classList.contains('fish-left') ? -200 : fishElement.classList.contains('fish-right') ? 200 : 0,
		y: fishElement.classList.contains('fish-top') ? -200 : 0,
		duration: 4,
		repeat: -1,
		yoyo: true,
		ease: "back.out"
...
class BaseComponent {
	...
	createOverlay() {
		...
		this.overlay.className =
			"fixed inset-0 bg-black opacity-50 z-40 hidden transition-opacity duration-300";
		document.body.appendChild(this.overlay);
	}

	addEventListeners() {
		document
			.querySelectorAll(`[data-${this.componentType}-target="${this.id}"]`)
			.forEach((button) => {
				button.addEventListener("click", () => this.open());
			});

		this.element
			.querySelectorAll(`[data-${this.componentType}-close="${this.id}"]`)
			.forEach((button) => {
...
class Tabs {
	constructor(element) {
		...
		if (!this.tabLinks.length) {
			console.warn(
				"No tab links found for this tab group. Skipping initialization.",
			);
			return;
		}
		if (!this.tabPanels.length) {
			console.warn(
				"No tab panels found for this tab group. Skipping initialization.",
			);
			return;
		}
		if (this.tabLinks.length !== this.tabPanels.length) {
			console.warn(
				"The number of tab links does not match the number of tab panels.",
			);
		}
	...
	constructor(accordion) {
		try {
			...
			if (this.items.length === 0) {
				throw new Error(
					`No accordion items found in accordion with class "${accordion}"`,
				);
			}
			this.init();
		} catch (error) {
	...
	constructor(menuSelector) {
		try {
			...
			if (this.menus.length === 0) {
				throw new Error(
					`No submenu items found with selector "${menuSelector}"`,
				);
			}
			this.init();
		} catch (error) {
...
if (document.querySelector(".banner1Slider")) {
	new swiper__WEBPACK_IMPORTED_MODULE_0__["default"](".banner1Slider", {
		modules: [swiper_modules__WEBPACK_IMPORTED_MODULE_1__.Autoplay, swiper_modules__WEBPACK_IMPORTED_MODULE_1__.EffectFade, swiper_modules__WEBPACK_IMPORTED_MODULE_1__.Pagination],
		slidesPerView: 1,
		autoplay: true,
		loop: true,
		effect: "fade",
...
if (document.querySelector(".projectSlider")) {
	new swiper__WEBPACK_IMPORTED_MODULE_0__["default"](".projectSlider", {
		slidesPerView: "auto",
		autoplay: {
			delay:1
		},
...
if (document.querySelector(".team1Slider")) {
	new swiper__WEBPACK_IMPORTED_MODULE_0__["default"](".team1Slider", {
		slidesPerView:1,	
		loop: true,		
...
if (document.querySelector(".testimonialSlider1")) {
	new swiper__WEBPACK_IMPORTED_MODULE_0__["default"](".testimonialSlider1", {
		slidesPerView:1,	
		loop: true,		
		modules: [swiper_modules__WEBPACK_IMPORTED_MODULE_1__.Autoplay,swiper_modules__WEBPACK_IMPORTED_MODULE_1__.Navigation],
		navigation:{
...
if (document.querySelector(".portfolioSlider2")) {
	new swiper__WEBPACK_IMPORTED_MODULE_0__["default"](".portfolioSlider2", {
		slidesPerView: "auto",		
		loop: true,		
...
if (document.querySelector(".testimonialSlider2")) {
	new swiper__WEBPACK_IMPORTED_MODULE_0__["default"](".testimonialSlider2", {
		slidesPerView:1,	
		loop: true,		
		modules: [swiper_modules__WEBPACK_IMPORTED_MODULE_1__.Autoplay,swiper_modules__WEBPACK_IMPORTED_MODULE_1__.Navigation],
		navigation:{
...
if (document.querySelector(".bannerSlider3")) {
	new swiper__WEBPACK_IMPORTED_MODULE_0__["default"](".bannerSlider3", {
		slidesPerView:1.2,	
		loop: true,		
		modules: [swiper_modules__WEBPACK_IMPORTED_MODULE_1__.Autoplay],
		spaceBetween:16,
  ...
  // blog slider
  if (document.querySelector('.serviceSlider3')) {
    new swiper__WEBPACK_IMPORTED_MODULE_0__["default"]('.serviceSlider3', {
      slidesPerView: 'auto',
      modules: [swiper_modules__WEBPACK_IMPORTED_MODULE_1__.Autoplay],
      autoplay:true,
      spaceBetween: 24,
...
if (document.querySelector(".thumbSlider")) {
	// game details slider
	var swiper = new swiper__WEBPACK_IMPORTED_MODULE_0__["default"](".thumbSlider", {
		spaceBetween: 16,
		slidesPerView: 3,
		freeMode: true,
		modules: [swiper_modules__WEBPACK_IMPORTED_MODULE_1__.Autoplay, swiper_modules__WEBPACK_IMPORTED_MODULE_1__.Thumbs, swiper_modules__WEBPACK_IMPORTED_MODULE_1__.FreeMode],
		autoplay: true,
		loop: true,
		watchSlidesProgress: true,
	});
	var swiper2 = new swiper__WEBPACK_IMPORTED_MODULE_0__["default"](".productSlider", {
		spaceBetween: 10,
		loop: true,
		autoplay: true,
		modules: [swiper_modules__WEBPACK_IMPORTED_MODULE_1__.Autoplay, swiper_modules__WEBPACK_IMPORTED_MODULE_1__.Thumbs],
		thumbs: {
			swiper: swiper,
		},
	});
...
/* harmony import */ var _animations__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./animations */ "./src/js/animations.js");
...
/* harmony import */ var slim_select__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! slim-select */ "./node_modules/slim-select/dist/slimselect.js");
/* harmony import */ var slim_select__WEBPACK_IMPORTED_MODULE_4___default = /*#__PURE__*/__webpack_require__.n(slim_select__WEBPACK_IMPORTED_MODULE_4__);
/* harmony import */ var _components__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(/*! ./components */ "./src/js/components.js");
/* harmony import */ var img_comparison_slider__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(/*! img-comparison-slider */ "./node_modules/img-comparison-slider/dist/index.js");
/* harmony import */ var img_comparison_slider__WEBPACK_IMPORTED_MODULE_6___default = /*#__PURE__*/__webpack_require__.n(img_comparison_slider__WEBPACK_IMPORTED_MODULE_6__);
...
document.addEventListener("DOMContentLoaded", () => {
	...
	tabGroups.forEach((tabGroup) => {
		new _components__WEBPACK_IMPORTED_MODULE_5__.Tabs(tabGroup);
	});

	// initialize accordions
	if (document.querySelector(".accordion")) {
		document.querySelectorAll(".accordion").forEach((el) => {
			new _components__WEBPACK_IMPORTED_MODULE_5__.Accordion(el);
		});
	}

	// initialize submenus
	if (document.querySelector(".submenu-item")) {
		new _components__WEBPACK_IMPORTED_MODULE_5__.Submenu(".submenu-item");
	}
...
if (selects.length) {
	selects.forEach((select) => {
		...
		new (slim_select__WEBPACK_IMPORTED_MODULE_4___default())({
			select,
			settings: {
				allowDeselect: true,
...
if (document.querySelector(".passwordInput")) {
	...
	inputs.forEach((input) => {
		const eye = input.querySelector(".eye");
		const eyeSlash = input.querySelector(".eye-off");
...
const startProgressAnimation = (progressBar) => {
	const progressValue = progressBar.querySelector(".percentage");
	let startValue = 0;
	const endValue = Number(progressBar.getAttribute("data-percentage"));
	const speed = 30;
	const progressColor =
		progressBar.getAttribute("data-progress-color") || "#00A8E8";
	const bgColor = progressBar.getAttribute("data-bg-color") || "#D9D9D9";
...
const startCountUp = (element) => {
	const target = +element.getAttribute("data-end-value");
	let current = 0;

	// Update the counter value over time
	...
	const interval = setInterval(() => {
		if (current < target) {
			...
			element.textContent = target; // Set the target value when done
	...
	counters.forEach((counter) => {
		...
		if (rect.top <= window.innerHeight && rect.bottom >= 0) {
			if (!counter.classList.contains("animated")) {
				counter.classList.add("animated"); // Add a class to prevent reanimation
...
checkInView(); // Initial check in case elements are already in view
