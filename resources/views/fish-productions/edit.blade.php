<x-app-layout title="Edit Fish Production" is-header-blur="true">
    <!-- Main Content Wrapper -->
    <main class="main-content w-full px-[var(--margin-x)] pb-8">
        <div class="mt-4 grid grid-cols-12 gap-4 sm:mt-5 sm:gap-5 lg:mt-6 lg:gap-6">
            <div class="col-span-12 flex justify-center">
                <div class="card w-full max-w-4xl" x-data="fishProductionEditForm()">
                    <div class="p-6">
                        <!-- Header -->
                        <div class="mb-6">
                            <div class="flex items-center mb-3">
                                ...
                                <h1 class="text-2xl font-semibold text-slate-800 dark:text-navy-50">Edit Fish Production</h1>
                            </div>
                            <p class="text-slate-500 dark:text-navy-200 text-sm">Update fish production information</p>
                        </div>
                        ...
                        <form action="{{ route('cms.fish-productions.update', $fishProduction->id) }}" method="POST" @submit="validateForm" class="space-y-6">
                            @csrf
                            @method('PUT')
    ...
    <script>
        function fishProductionEditForm() {
            return {
                formData: {
                    species: '{{ old('species', $fishProduction->species) }}',
                    weight_range: '{{ old('weight_range', $fishProduction->weight_range) }}',
                    rate: '{{ old('rate', $fishProduction->rate) }}',
                    fish_qty: '{{ old('fish_qty', $fishProduction->fish_qty) }}',
                    total_weight: '{{ old('total_weight', $fishProduction->total_weight) }}',
                    avg_weight: '{{ old('avg_weight', $fishProduction->avg_weight) }}'
                },
