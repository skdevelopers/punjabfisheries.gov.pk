    ...
    <section class="pt-32 pb-20 bg-primary-50">
      <div class="cont">
        <div class="text-center">
          ...
          <h1 class="text-4xl xl:text-5xl font-bold text-neutral-900 mb-4">{{ $announcement->title }}</h1>
          <p class="text-lg text-neutral-600 max-w-3xl mx-auto">{{ $announcement->description }}</p>
          <div class="flex items-center justify-center gap-6 mt-6 text-sm text-neutral-500">
            <span>Published: {{ $announcement->published_date->format('M d, Y') }}</span>
    ...
    <section class="py-120 bg-neutral-0">
      <div class="cont">
        <div class="max-w-4xl mx-auto">
          <div class="bg-neutral-0 rounded-2xl p-8 xl:p-12 shadow-lg border border-neutral-40">
            ...
            <div class="mt-8 pt-8 border-t border-neutral-40">
              <h3 class="text-lg font-semibold text-neutral-900 mb-4">Attachments</h3>
              <a href="{{ Storage::url($announcement->file_path) }}" target="_blank" class="inline-flex items-center gap-3 bg-primary-50 text-primary-700 px-6 py-3 rounded-lg hover:bg-primary-100 transition-colors">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                </svg>
                <span>{{ $announcement->file_name ?? 'Download File' }}</span>
              </a>
            </div>
            @endif
            ...
            <div class="mt-8 pt-8 border-t border-neutral-40">
              <h3 class="text-lg font-semibold text-neutral-900 mb-4">External Link</h3>
              <a href="{{ $announcement->external_url }}" target="_blank" class="inline-flex items-center gap-3 bg-secondary text-neutral-0 px-6 py-3 rounded-lg hover:bg-secondary/90 transition-colors">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div class="mt-16">
            <h2 class="text-2xl xl:text-3xl font-bold text-neutral-900 mb-8">Related Announcements</h2>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              @foreach($relatedAnnouncements as $related)
              <div class="bg-neutral-0 rounded-2xl p-6 shadow-lg hover:shadow-xl transition-all duration-300 border border-neutral-40 group">
                <div class="flex items-center gap-3 mb-4">
                  <div class="w-3 h-3 bg-{{ $related->priority === 'high' ? 'red' : ($related->priority === 'normal' ? 'primary' : 'blue') }}-500 rounded-full"></div>
                  <span class="text-sm font-semibold text-{{ $related->priority === 'high' ? 'red' : ($related->priority === 'normal' ? 'primary' : 'blue') }}-600 uppercase">{{ ucfirst($related->type) }}</span>
                </div>
                <h3 class="text-lg font-bold mb-3 text-neutral-900 line-clamp-2 group-hover:text-primary-500 transition-colors">{{ $related->title }}</h3>
                <p class="text-neutral-600 mb-4 line-clamp-3">{{ Str::limit($related->description, 100) }}</p>
                <div class="flex items-center justify-between">
                  <span class="text-sm text-neutral-500">{{ $related->published_date->format('M d, Y') }}</span>
                  <a href="{{ route('frontend.announcements.show', $related) }}" class="text-primary-500 text-sm font-medium hover:text-primary-600">Read More →</a>
                </div>
              </div>
              @endforeach
            </div>
