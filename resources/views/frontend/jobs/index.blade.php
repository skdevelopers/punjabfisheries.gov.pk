      ...
      <div class="cont">
        @if($jobs->count() > 0)
          <div class="bg-white rounded-lg shadow-lg p-6">
            <h2 class="text-3xl font-bold text-red-600 mb-8 text-center underline">Career Opportunities</h2>
            <div class="space-y-4">
                  ...
                  <div class="flex items-start justify-between">
                    <div class="flex-1">
                      <div class="flex items-center gap-2 mb-2">
                        @if($career->created_at->isAfter(now()->subDays(7)))
                          <span class="bg-red-100 text-red-800 text-xs font-semibold px-2 py-1 rounded">(New)</span>
                        @endif
                        <h3 class="text-lg font-semibold text-gray-900">{{ $career->title }}</h3>
                      </div>
                      <div class="text-sm text-gray-600 mb-3">
                        @if($career->department)
                          <span class="font-medium">Department:</span> {{ $career->department }} |
                      @endif
                      @if($career->location)
                          <span class="font-medium">Location:</span> {{ $career->location }} |
                      @endif
                      @if($career->employment_type)
                          <span class="font-medium">Type:</span> {{ $career->employment_type }} |
                        @endif
                        <span class="font-medium">Posted:</span> {{ $career->created_at->format('d-m-Y') }}
                      </div>
                      <p class="text-gray-700 mb-3">{{ Str::limit($career->description, 200) }}</p>
                      <div class="flex items-center gap-4">
                        <a href="{{ route('frontend.jobs.show', $career) }}"
                        ...
                        @if($career->hasAttachment())
                          <a href="{{ $career->attachment_url }}"
                            ...
                            Download {{ strtoupper($career->attachment_type) }}
                          </a>
                      @endif
                      </div>
                    </div>
                    <div class="ml-4">
                      <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                        @if($career->status === 'open') bg-green-100 text-green-800
                        @elseif($career->status === 'closed') bg-red-100 text-red-800
        ...
        @else
          <div class="text-center py-20">
            ...
            <h3 class="text-2xl font-semibold text-neutral-700 mb-4">No Career Openings Available</h3>
            <p class="text-neutral-500 mb-8">We currently don't have any career openings. Please check back later for new opportunities.</p>
