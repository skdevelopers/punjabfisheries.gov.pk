<x-app-layout title="Create Target" is-header-blur="true">
    <main class="main-content w-full px-[var(--margin-x)] pb-8">
        ...
        <div class="card">
            <div class="p-4 sm:p-5">
                <form action="{{ route('crm.targets.store') }}" method="POST">
                    @csrf
                    ...
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        ...
                        <div>
                            ...
                            <input type="number" name="target_value" value="{{ old('target_value') }}" step="0.01" min="0" required
                        ...
                        <div>
                            ...
                            <select name="unit" required
                                ...
                                @foreach($units as $key => $label)
                                    <option value="{{ $key }}" {{ old('unit', 'kg') === $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                            @error('unit')
                                <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        ...
                        <div>
                            ...
                            <select name="priority" required
                                ...
                                @foreach($priorities as $key => $label)
                                    <option value="{{ $key }}" {{ old('priority', '2') === (string)$key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                            @error('priority')
                                <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <textarea name="description" rows="3"
                                ...
                                placeholder="Enter target description...">{{ old('description') }}</textarea>
                            @error('description')
                                <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                            @enderror
                        </div>
