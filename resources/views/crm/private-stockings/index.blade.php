<x-app-layout title="Private Stocking" is-header-blur="true">
    <!-- Main Content Wrapper -->
    <main class="main-content w-full px-[var(--margin-x)] pb-8">
        <div class="mt-4 grid grid-cols-12 gap-4 sm:mt-5 sm:gap-5 lg:mt-6 lg:gap-6">
            <div class="col-span-12">
                <div class="card" x-data="privateStockingIndex()">
                    <div class="p-6">
                        <!-- Header -->
                        <div class="flex items-center justify-between mb-6">
                            <div>
                                <h1 class="text-2xl font-semibold text-slate-800 dark:text-navy-50">Private Stocking</h1>
                                <p class="text-slate-500 dark:text-navy-200 text-sm">Manage private stocking records</p>
                            </div>
                            <a href="{{ route('crm.private-stockings.create') }}" 
                            ...
                            <table class="w-full">
                                <thead class="border-b border-slate-200 dark:border-navy-500">
                                    <tr>
                                        ...
                                        <th class="whitespace-nowrap px-4 py-3 text-left text-xs font-medium text-slate-500 dark:text-navy-300 uppercase tracking-wider">Income from Fish Seed</th>
                                ...
                                <tbody class="divide-y divide-slate-200 dark:divide-navy-500">
                                    @forelse($privateStockings as $stocking)
                                        <tr x-data="{ isDeleting: false }" x-show="!isDeleting" x-transition:leave="transition ease-in duration-300" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="hover:bg-slate-50 dark:hover:bg-navy-600 transition-colors">
                                            ...
                                            <td class="px-4 py-4 whitespace-nowrap text-sm text-slate-900 dark:text-navy-50">PKR {{ number_format($stocking->income_from_fish_seed, 2) }}</td>
                                            ...
                                            <td class="px-4 py-4 whitespace-nowrap text-sm font-medium">
                                                <div class="flex items-center space-x-2">
                                                    <a href="{{ route('crm.private-stockings.show', $stocking->id) }}" 
                                                    ...
                                                    <a href="{{ route('crm.private-stockings.edit', $stocking->id) }}" 
                                        ...
                                        <tr>
                                            <td colspan="6" class="px-4 py-12 text-center">
                                                <div class="text-slate-500 dark:text-navy-300">
                                                    ...
                                                    <p class="text-lg font-medium">No private stocking records found</p>
                        ...
                        <!-- Pagination -->
                        @if($privateStockings->hasPages())
                            <div class="mt-6">
                                {{ $privateStockings->links() }}
                            </div>
                        @endif
                    </div>
                </div>
        ...
        function privateStockingIndex() {
            return {
                ...
                async deleteRecord(id, rowElement) {
                    ...
                    try {
                        const response = await axios.delete(`/crm/private-stockings/${id}`, {
                            headers: {
