<x-app-layout title="Career Details" is-header-blur="true">
    ...
    <main class="main-content w-full px-[var(--margin-x)] pb-8">
        <div class="mt-4 sm:mt-5 lg:mt-6">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4">
                <div>
                    <h1 class="text-2xl font-semibold text-slate-800 dark:text-navy-50">Career Details</h1>
                    <p class="text-slate-500 dark:text-navy-200">View and manage career posting details</p>
                </div>
                <div class="mt-4 sm:mt-0 flex space-x-2">
                    <a href="{{ route('cms.jobs.edit', $career) }}" class="btn bg-primary font-medium text-white hover:bg-primary-focus focus:bg-primary-focus active:bg-primary-focus/90 dark:bg-accent dark:hover:bg-accent-focus dark:focus:bg-accent-focus dark:active:bg-accent/90">
            ...
            <div class="grid grid-cols-1 gap-4 lg:grid-cols-3">
                <div class="lg:col-span-2">
                    <div class="card">
                        <div class="p-4 sm:p-5">
                            <div class="mb-6">
                                <h2 class="text-2xl font-bold text-slate-800 dark:text-navy-50 mb-4">{{ $career->title }}</h2>
                                    ...
                                    <div>
                                        ...
                                        <p class="text-slate-700 dark:text-navy-100">
                                            @if($career->experience_level)
                                                <span class="badge bg-slate-100 text-slate-600">{{ $career->experience_level }} Level</span>
                                            @else
                                                N/A
                                            @endif
                                        </p>
                                ...
                                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 mb-4">
                                    <div>
                                        ...
                                        <p class="text-slate-700 dark:text-navy-100 font-semibold">{{ $career->salary_range }}</p>
                                    </div>
                                    <div>
                                        <span class="text-sm font-medium text-slate-500 dark:text-navy-200">Application Deadline:</span>
                                        <p class="text-slate-700 dark:text-navy-100">
                                            {{ $career->application_deadline ? $career->application_deadline->format('M d, Y') : 'No deadline set' }}
                                        </p>
                                    </div>
                                </div>
                                ...
                                <div class="mb-6">
                                    <h3 class="text-lg font-semibold text-slate-800 dark:text-navy-50 mb-3">Career Description</h3>
                ...
                <div class="lg:col-span-1">
                    <div class="card">
                        <div class="p-4 sm:p-5">
                            <h3 class="text-lg font-semibold text-slate-800 dark:text-navy-50 mb-4">Career Information</h3>

                            <div class="space-y-4">
                                <div>
                                    <span class="text-sm font-medium text-slate-500 dark:text-navy-200">Career ID:</span>
                                    <p class="text-slate-700 dark:text-navy-100 font-mono">#{{ $career->id }}</p>
                                </div>

                                <div>
                                    <span class="text-sm font-medium text-slate-500 dark:text-navy-200">Created:</span>
                                    <p class="text-slate-700 dark:text-navy-100">{{ $career->created_at->format('M d, Y \a\t g:i A') }}</p>
                                </div>

                                <div>
                                    <span class="text-sm font-medium text-slate-500 dark:text-navy-200">Last Updated:</span>
                                    <p class="text-slate-700 dark:text-navy-100">{{ $career->updated_at->format('M d, Y \a\t g:i A') }}</p>
                                </div>
                            </div>

                            <div class="mt-6 space-y-3">
                                <form action="{{ route('cms.jobs.toggle-status', $career) }}" method="POST">
                                    @csrf
                                    @method('PATCH')
                                    <button type="submit" class="btn w-full {{ $career->status === 'open' ? 'bg-warning font-medium text-white hover:bg-warning-focus' : 'bg-success font-medium text-white hover:bg-success-focus' }}">
                                        <svg class="size-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        ...
                                        {{ $career->status === 'open' ? 'Close Career' : 'Open Career' }}
                                    </button>
                                </form>

                                <form action="{{ route('cms.jobs.toggle-active', $career) }}" method="POST">
                                    @csrf
                                    @method('PATCH')
                                    <button type="submit" class="btn w-full {{ $career->is_active ? 'bg-slate-150 font-medium text-slate-800 hover:bg-slate-200' : 'bg-success font-medium text-white hover:bg-success-focus' }}">
                                        <svg class="size-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <a href="{{ route('frontend.jobs.show', $career) }}" class="btn w-full bg-info font-medium text-white hover:bg-info-focus" target="_blank">
                                    <svg class="size-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <div class="mt-6 pt-6 border-t border-slate-200 dark:border-navy-500">
                                <form action="{{ route('cms.jobs.destroy', $career) }}" method="POST"
                                      onsubmit="return confirm('Are you sure you want to delete this career? This action cannot be undone.')">
                                    @csrf
                                    @method('DELETE')
