<x-app-layout title="Edit Job" is-header-blur="true">
    ...
    <main class="main-content w-full px-[var(--margin-x)] pb-8">
        <div class="mt-4 sm:mt-5 lg:mt-6">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4">
                <div>
                    <h1 class="text-2xl font-semibold text-slate-800 dark:text-navy-50">Edit Job: {{ $job->title }}</h1>
                    <p class="text-slate-500 dark:text-navy-200">Update job posting information</p>
                </div>
            ...
            <div class="card">
                <div class="p-4 sm:p-5">
                    <form action="{{ route('cms.jobs.update', $job) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                                ...
                                <div class="space-y-4">
                                    ...
                                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                                        <div>
                                            ...
                                            <input type="number" 
                                                   ...
                                                   id="salary_min" name="salary_min" value="{{ old('salary_min', $job->salary_min) }}" min="0" step="0.01">
                                            @error('salary_min')
                                                <p class="mt-1 text-sm text-error">{{ $message }}</p>
                                            @enderror
                                        </div>
                                        <div>
                                            ...
                                            <input type="number" 
                                                   ...
                                                   id="salary_max" name="salary_max" value="{{ old('salary_max', $job->salary_max) }}" min="0" step="0.01">
                                            @error('salary_max')
                                                <p class="mt-1 text-sm text-error">{{ $message }}</p>
                                            @enderror
                                        </div>
                                    ...
                                    <div>
                                        ...
                                        <textarea class="form-textarea w-full @error('requirements') border-error @enderror" 
                                                  id="requirements" name="requirements" rows="4">{{ old('requirements', $job->requirements) }}</textarea>
                                        @error('requirements')
                                            <p class="mt-1 text-sm text-error">{{ $message }}</p>
                                        @enderror
                                    </div>
                                        ...
                                        @if($job->hasAttachment())
                                            <div class="mt-2 p-3 bg-green-50 border border-green-200 rounded-md">
                                                <p class="text-sm text-green-800">
                                                    ...
                                                    <a href="{{ $job->attachment_url }}" target="_blank" class="text-green-600 hover:text-green-800 underline">
                                                        {{ $job->attachment_name }}
                                                    </a>
                                                    ({{ strtoupper($job->attachment_type) }})
                                                </p>
                                                <p class="text-xs text-green-600 mt-1">Upload a new file to replace the current one</p>
                                            </div>
                                        @endif
                                        @error('attachment')
                                            <p class="mt-1 text-sm text-error">{{ $message }}</p>
